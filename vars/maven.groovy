...
def call(String pipeliType){
    ...
    if(bandera){
        ...
        if(str.contains('nexusUpload') || params.stage.isEmpty())
        {
            stage('nexusUpload') {
                 figlet 'Nexus Upload'
                bat "curl -v --user admin:123456 --upload-file C:/Users/nmt02/.jenkins/workspace/Laboratorio_Final/pipeline-release/build/pipeline-release-0.0.1.jar http://localhost:8081/repository/lab-m3/com/pipeline-release/pipeline-release/0.0.1/pipeline-release-0.0.1.jar "
            } 
        }
